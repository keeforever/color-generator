...
function App() {
  ...
  useEffect(() => {
    colorConversion()
    const time=setTimeout(()=>{setIsError(false)}, 2000);
    return ()=>{
      clearTimeout(time)
    }
  }, [isError])
      ...
      <section className="container">
        <header className='head'>
            ...
            <form className='form-control' onSubmit={colorSubmit}>
              <input type="text" placeholder="red" className={`form-color-input ${isError ? 'error-alert': ''}` }value={colorValue==='' ?  '':colorValue } onChange={inputHandler}/>
